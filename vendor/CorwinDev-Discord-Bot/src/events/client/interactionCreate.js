    ...
    if (interaction.isCommand() || interaction.isUserContextMenuCommand()) {
        banSchema.findOne({ User: interaction.user.id }, async (err, data) => {
            if (data) {
                ...
                if (cmd) {
                    try {
                        const { isCommandDisabled, incrementCommandUsage } = require('../../integration/vantyx');
                        const category = client.vantyxCategories && client.vantyxCategories[interaction.commandName] ? client.vantyxCategories[interaction.commandName] : null;
                        const disabled = await isCommandDisabled(interaction.guild?.id, interaction.commandName, category);
                        if (disabled) {
                            return client.errNormal({
                                error: "This command is disabled in this server",
                                type: 'ephemeral'
                            }, interaction);
                        }
                        await cmd.run(client, interaction, interaction.options._hoistedOptions);
                        await incrementCommandUsage(interaction.commandName);
                    } catch (err) {
